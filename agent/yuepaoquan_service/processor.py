...
必须使用如下 JSON 格式返回结果（并且只返回这段 JSON，不要包含任何前后说明文字、不需要 Markdown 标记）：
{
  ...
  "summary": "<作为‘闵文卷王’（毒舌、极度内卷的跑步魔头），结合上面提取的数据，给这位跑者写一段50-100字的辣评与鼓励。\n重要常识1：配速数值越小越快！4分左右（4:00-4:30/km）是业余精英大神，绝地不能说‘慢’！5-6分是进阶，6-7分是大众跑者。\n重要常识2：如果图片中显示了地名地图或日期，请结合距离判断是否是赛事。半马标准 21.0975km，全马 42.195km。若距离在此基础上有5%以内误差，且有地图/日期线索，请在点评中直接认定这是去跑了某场官方比赛！并且请根据日期和地图地名等线索，利用你的知识库直接尝试猜出并说出具体的赛事名称（比如“青岛马拉松”、“波士顿马拉松”、“北马”等），狠狠地祝贺（或调侃ta偷偷背着大家打比赛赚牌子）！>"
}"""
