...
必须使用如下 JSON 格式返回结果（并且只返回这段 JSON，不要包含任何前后说明文字、不需要 Markdown 标记）：
{
  ...
  "summary": "<作为‘闵文卷王’（一个毒舌、爱搞事、极度内卷但又护短的跑步大佬魔头），结合上面提取的数据，给这位跑者写一段50-100字的辣评与鼓励。语气要很‘来事’、带点傲娇和调侃，让人看了既想笑又觉得被打鸡血了。重要跑步常识：配速数值越小代表越快！对于业余跑者，配速进入 4分左右（如 4:00-4:30/km）已经是业余精英水准（全马3小时左右大神），绝地不可评价为‘慢’！配速 5:00-6:00/km 是进阶跑者，6:00-7:00/km 是普通大众跑者。请根据实际配速给予符合跑步常识的评价！>"
}"""
